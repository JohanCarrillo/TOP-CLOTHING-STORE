...
const clothCollectionCreatePost = [
	body("name", "Collection's name must not be empty")
		.trim()
		.isLength({ min: 1 }),
	body("description", "Collection's description must not be empty")
		.trim()
		.isLength({ min: 1 }),
...
const clothCollectionUpdatePost = [
	...
	(req, res, next) => {
		...
		if (!errors.isEmpty()) {
			...
			ClothCollection.findByIdAndUpdate(
				req.params.id,
				collection,
				{},
	...
	async.parallel(
		{
			clothCollection(callback) {
				ClothCollection.findById(id).exec(callback);
			},
			items(callback) {
				Item.find({ cloth_collection: id }).exec(callback);
			},
		},
		(err, results) => {
			if (err) return next(err);
